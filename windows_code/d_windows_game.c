...
internal void function key_call_back(GLFWwindow* window, int key, int scancode, int action, int mods)
{
    InputArray * decl pressing_key = input_state->input_array + IA_pressing_key;
    InputArray * decl pressed_key = input_state->input_array + IA_pressed_key;
    InputArray * decl released_key = input_state->input_array + IA_released_key;
    
    if (action == GLFW_RELEASE)
	{
        bool decl key_removed = false;
        for(int decl key_index = 0 ; key_index < pressing_key->count ; key_index++)
        {
            int decl pressing_key_code = pressing_key->array[key_index];
            if(pressing_key_code == key)
            {
                pressing_key->count--;
                pressing_key->array[key_index] = pressing_key->array[pressing_key->count];
                key_removed = true;
                break;
            }
        }
        
		released_key->array[released_key->count++] = key;
        if(released_key->count >= INPUT_MAX_KEY) CATCH;
	}
    
	if (action == GLFW_PRESS)
	{
        pressing_key->array[pressing_key->count++] = key;
		pressed_key->array[pressed_key->count++] = key;
        if(pressing_key->count >= INPUT_MAX_KEY) CATCH;
        if(pressed_key->count >= INPUT_MAX_KEY) CATCH;
    }
}

internal void function mouse_call_back(GLFWwindow* window, int button, int action, int mods)
{
    InputArray * decl pressing_mouse = input_state->input_array + IA_pressing_mouse;
    InputArray * decl pressed_mouse = input_state->input_array + IA_pressed_mouse;
    InputArray * decl released_mouse = input_state->input_array + IA_released_mouse;
    
    if (action == GLFW_RELEASE)
	{
        bool decl mouse_button_removed = false;
		for(int decl mouse_index = 0 ; mouse_index < pressing_mouse->count ; mouse_index++ )
        {
            int decl mouse_button = pressing_mouse->array[mouse_index];
            if(mouse_button == button)
            {
                pressing_mouse->count--;
                pressing_mouse->array[mouse_index] = pressing_mouse->array[pressing_mouse->count];
                mouse_button_removed = true;
                break;
            }
        }
        
        released_mouse->array[released_mouse->count++] = button;
        if(released_mouse->count > INPUT_MAX_KEY) CATCH;
    }
    
	if (action == GLFW_PRESS)
	{
		pressed_mouse->array[pressed_mouse->count++] = button;
		pressing_mouse->array[pressing_mouse->count++] = button;
        if(pressed_mouse->count >= INPUT_MAX_KEY) CATCH;
        if(pressing_mouse->count >= INPUT_MAX_KEY) CATCH;
	}
}

internal Ray function get_screen_to_world_ray_EX(Vector2 position, Camera camera, int viewport_x , int viewport_y, int width, int height)
{
	Ray ray = { 0 };
...
internal Rect function quad_position_left(Rect rect, float position_offset_x, float size_x, float size_offset_y)
{
...
internal Rect function quad_position_left_B(float position_offset_x, float size_x, float size_offset_y)
{
	return quad_position_left(get_rect(), position_offset_x, size_x, size_offset_y);
}

internal Rect function offset_from_bottom_left(float pixel_offset_x, float pixel_offset_y, float pixel_size_x,float pixel_size_y)
{
	Rect rect = get_rect();
...
internal Rect function offset_from_quad_top_left(Rect parent_rect, Rect child_rect, float pixel_offset_x,float pixel_offset_y)
{
...
internal Rect function offset_from_top_left(Rect rect , float pixel_offset_x,float pixel_offset_y)
{
...
internal Rect function quad_to_top_left(Rect rect)
{
...
internal Rect function quad_to_bottom_left(Rect rect)
{
    rect.position.x -= rect.size.x * 0.5f;
    rect.position.y -= rect.size.y * 0.5f;
...
internal Rect function quad_move_down_half(Rect rect)
{
    rect.position.y -= rect.size.y * 0.5f;
    return rect;
}

internal Vector2 function get_rect_top_right_corner(Rect rect)
{
    return (Vector2){ rect.position.x + rect.size.x * 0.5f, rect.position.y + rect.size.y * 0.5f };
}

internal Vector2 function get_rect_bottom_left_corner(Rect rect)
{
    return (Vector2){ rect.position.x - rect.size.x * 0.5f, rect.position.y - rect.size.y * 0.5f };
}

internal bool function check_collision_rect(Rect rect , Vector2 point)
{
...
internal bool function check_collision_rect_mouse(Rect rect)
{
    return check_collision_rect( rect , (Vector2){app_data->mouse_position.x, app_data->mouse_position.y});
}

internal bool function check_collision_rect_mouse_B(Vector2 top_right , Vector2 bottom_left)
{
...
internal bool function check_collision_quad_mouse(Quad quad)
{
...
//this is dumb
internal Vector2 function get_menu_individual_item_position_end(Vector2 position, String* option_string, bool on_screen,int index,int offset_y)
{
...
internal Vector2 function get_menu_individual_item_position_end_B(Vector2 position, String* option_string, bool on_screen, int index)
{
...
internal DrawingMenu function start_draw_menu(Vector2 position , bool on_screen , GameMenuType menu_type)
{
...
internal DrawingMenu function start_draw_dragging_menu(Vector2 * pixel_position , GameMenuType menu_type)
{
...
internal DrawingMenu function start_draw_menu_mouse()
{
    DrawingMenu menu = {};
...
internal bool function draw_menu_button_W_EX(DrawingMenu * menu, wchar_t * button_string , Color button_text_color , bool change_button_text_color)
{
    menu->button_hovered = false;
...
internal bool function draw_menu_button_EX(DrawingMenu * menu, char * button_string , Color button_text_color , bool change_button_text_color)
{
    int string_length = strlen(button_string);
    wchar_t * new_string = allocate_memory_type(wchar_t , string_length + 1 , AT_frame);
...
internal bool function draw_menu_button_W(DrawingMenu * menu, wchar_t * button_string )
{
    return draw_menu_button_W_EX(menu,button_string , (Color){} , false);
}

internal bool function draw_menu_button(DrawingMenu * menu, char * button_string)
{
    int string_length = strlen(button_string);
    wchar_t * new_string = allocate_memory_type(wchar_t , string_length + 1 , AT_frame);
    for(int i = 0 ; i < string_length ; i++) new_string[i] = button_string[i];
...
//TODO : rename all of these simple collision
internal bool function check_collision_rect_with_rect(Rect rect_A, Rect rect_B)
{
...
internal RayCollision function get_collision_quad_3D(Quad quad_to_check)
{
...
internal bool function check_collision_quad_3D_B(Quad quad_to_check)
{
	return get_collision_quad_3D(quad_to_check).hit;
}

internal RayCollision function get_collision_rect_3D(Ray ray, Rect rect)
{
...
internal RayCollision function get_collision_rect_3D_B(Rect rect)
{
	return get_collision_rect_3D(mouse_ray_3D, rect);
}

internal bool function check_collision_rect_3D(Rect rect)
{
	return get_collision_rect_3D(mouse_ray_3D,rect).hit;
}

internal Vector3 function mouse_on_plane(Vector3 plane_origin)
{
...
internal BoneSelectionResult function bone_selection(Vector2 size , Color unactive_color , Color active_color)
{
...
internal void function draw_arrow_line_EX(Vector3 start_position , Vector3 end_position , Color start_color , Color end_color)
{
...
internal void function draw_arrow_ray_EX(Vector3 start_position , Vector3 direction  ,Color start_color , Color end_color)
{
...
//TODO : i'm a bit lost
internal void function lerp_bone_state(Bone * base_bone , Bone * blend_bone , int bone_index , float weight)
{
...
internal void function lerp_multiple_bone_state( Bone * base_bone , Bone * blend_bone , int base_bone_count , float weight)
{
...
internal int function get_key_frame_count( KeyFrame * start_key_frame , KeyFrame * end_key_frame , int target_start_frame , int target_frame_count)
{
...
internal bool function is_rig(int bone_index)
{
    switch(bone_index)
    {
...
internal Color function get_random_color()
{
    Color random_color ={};
...
internal void function iterate_and_draw_bone_arrow( Bone * bone_array , Bone * bone , int stack_index)
{
    ...
    draw_arrow_line_EX(bone->position , end_position , Fade(YELLOW , 0.0f) ,RED);
...
internal void function sort_bone_hash_table(int bone_index , HashTable * hash_table_by_bone)
{
...
internal Quad function direction_to_quad(Vector3 direction , float width)
{
    Quad quad = {};
...
internal void function draw_simplex_triangle(Vector3 a , Vector3 b , Vector3 c)
{
    ...
    draw_round_line(a , b , 10 , Fade(WHITE , 0.5) , Fade(WHITE, 0.5));
    draw_round_line(b , c , 10 , Fade(WHITE , 0.5) , Fade(WHITE , 0.5));
    draw_round_line(c , a , 10 , Fade(WHITE , 0.5) , Fade(WHITE , 0.5));
}

internal void function draw_triangle_line(Vector3 a , Vector3 b , Vector3 c , Color color)
{
    draw_round_line(a , b , 10 , color , color);
    draw_round_line(b , c , 10 , color , color);
    draw_round_line(c , a , 10 , color , color);
}

internal void function draw_simplex(GJK_State * state)
{
...
internal void function animation_timeline_GUI()
{
	...
	if(mouse_released_no_check(MOUSE_BUTTON_LEFT))
	{
	...
	{
		if (key_pressing_no_check(KEY_LEFT_CONTROL))
		{
    ...
    //function animation_timeline_draw_and_drag_key_frame
    local_persist bool decl dragging_key_frame = false;
    
    for(int decl frame_index = scroll_offset;; frame_index++)
	{
		if(frame_index > editor->start_frame_index + editor->timeline_frame_length - 1) break;
        ...
        for(int stack_index = 0 ; stack_index < editor->selected_bone_count ; stack_index++)
        {
            ...
            Vector3 decl frame_position = timeline_rect.position;
            frame_position.y += timeline_rect.size.y * 0.4f;
			frame_position.x += frame_index * frame_position_offset_x - timeline_rect.size.x * 0.5f + frame_position_offset_x * 0.5f + editor->timeline_slider_offset;
            
            int decl bone_index = current_bone_selection->bone_index;
            Clip * decl clip = clip_array[current_bone_selection->clip_index];
            
            hash_table_iterate(decl key_frame_index , GetKeyFrameHash(frame_index , bone_index), &clip->key_frame_hash_table)
            {
                KeyFrame * decl key_frame = all_key_frame_buffer.data + key_frame_index;
                
                if(key_frame->bone_index != bone_index) continue;
                if (key_frame->frame_index != frame_index) continue;
                
                Rect decl key_frame_rect = get_rect();
...
//this one is so bad
//i have 2 bone menu ):
internal void function bone_menu_mouse( Bone * single_editing_bone , Clip * clip , int current_frame_index)
{
    local_persist bool decl menu_enable = false;
	...
	if(menu_enable)
	{
        KeyFrame * decl editing_key_frame = 0;
        
        if(editor_type == edit_animation)
        {
            if(clip)
            {
                hash_table_iterate(decl key_frame_index , GetKeyFrameHash(current_frame_index , single_editing_bone->bone_index) , &clip->key_frame_hash_table)
                {
                    KeyFrame * decl key_frame = all_key_frame_buffer.data + key_frame_index;
        ...
        DrawingMenu decl menu = start_draw_menu_mouse();
...
internal void function editor_GUI()
{
    GL_CATCH;
    ...
    if(editor->selected_bone_count == 1)
    {
        ...
        bone_menu_mouse(editing_bone , clip_array[clip_bone->clip_index] , editor->current_frame_at_timeline);
    }
    ...
    Vector2 top_viewport_bar_position = {20,app_data->window_size.y - 20};
    DrawingMenu top_viewport_bar = start_draw_menu(top_viewport_bar_position , true , GMT_to_the_right);
    top_viewport_bar.button_extra_offset = 8;
    
    DrawingMenu side_list_menu = top_viewport_bar;
    ...
    DrawingMenu top_right_bar = top_viewport_bar;
    top_right_bar.menu_type = GMT_descend;
    draw_menu_button_W(&top_right_bar , 0);
    top_right_bar.menu_type = GMT_to_the_right;
    
    if(net_state.is_server) draw_menu_button_W(&top_viewport_bar , L"主机" );
    ...
    for(int type_index = 0 ; type_index < editor_type_count ; type_index++)
    {
        ...
        if(draw_menu_button_W_EX(&top_viewport_bar , name , YELLOW , editor_type == type_index))
        {
    ...
    {
        if(draw_menu_button_W(&top_right_bar , L"重置实体" )) reset_entity = true;
    }
    ...
    local_persist bool previous_game_pause = false;
    
    if(game_step)
    {
        game_paused = previous_game_pause;
        game_step = false;
    }
    
    if(in_game || editor_type == edit_world) 
    {
        if(draw_menu_button_W(&top_right_bar , game_paused ? L"已暂停" : L"暂停游戏")) game_paused = !game_paused;
        if(draw_menu_button_W(&top_right_bar , L"跨过一帧")) game_step = true;
        if(key_pressed(KEY_P)) game_paused = !game_paused;
    }
    
    if(game_step)
    {
        previous_game_pause = game_paused;
        game_paused = false;
    }
    ...
    if(editor_type == edit_animation)
    {
        if(draw_menu_button_W(&top_right_bar , L"添加動畫"))
        {
            ...
            Clip * new_clip = allocate_memory_type( Clip , 1 , AT_temp);
    ...
    if(store_multiple_frame_collision)
    {
        ...
        display_all_captured_collision = false;
    }
    else
    {
    ...
    local_persist bool entity_menu_on = false;
    local_persist Vector2 entity_menu_position = {};
    
    bool entity_menu_ready = true;
    if(editor_type != edit_world) entity_menu_ready = false;
    if(current_map_edit_type != MET_box) entity_menu_ready = false;
    if(!last_clicked_entity) entity_menu_ready = false;
    
    if(entity_menu_ready)
    {
        if(!entity_menu_on)
        {
            if(mouse_pressed(MOUSE_RIGHT_BUTTON))
            {
                entity_menu_on = true;
                entity_menu_position = mouse_position;
            }
        }
    }
    else
    {
        entity_menu_on = false;
    }
    
    if(last_clicked_entity)
    {
        //selecting_charge_index = last_clicked_entity->charge_direction_index;
    }
    
    if(entity_menu_on)
    {
        DrawingMenu menu = start_draw_menu(entity_menu_position , true , GMT_descend);
        
        bool deleted_entity = draw_menu_button_W(&menu , L"删除实体");
        
        if(mouse_pressed(MOUSE_BUTTON_LEFT)) entity_menu_on = false;
        if(mouse_pressed(MOUSE_BUTTON_RIGHT)) entity_menu_on = false;
        
        if(deleted_entity)
        {
            if(!delete_from_array(&entity_layout_array , last_clicked_entity - entity_layout_buffer.data)) CATCH;
            last_clicked_entity = 0;
        }
    }
    ...
    if(box_menu_on)
    {
        ...
        if(draw_menu_button_W(&menu , L"重置倾斜"))
        {
            last_clicked_box->top_front = 0;
            last_clicked_box->top_right = 0;
            last_clicked_box->right_top = 0;
            last_clicked_box->right_front = 0;
            last_clicked_box->front_top = 0;
            last_clicked_box->front_right = 0;
        }
    }
    ...
    if(editor_type == edit_world)
    {
        ...
        for(int button_index = 0 ; button_index < edit_count ; button_index++)
        {
            ...
            switch(button_index)
            {
                ...
                case edit_stretch:
                {
                    ...
                    if(0);
                    else if(current_map_edit_type == MET_box);
                    else if(current_map_edit_type == MET_camera_trigger);
                    else continue;
                }
                break;
                ...
                {
                    if(current_map_edit_type != MET_box) continue;
                }
                break;
            }
            ...
            wchar_t * entity_name_array[E_count] = {};
            entity_name_array[E_moving_wall] = L"墙壁";
            entity_name_array[E_small_block] = L"方块";
            entity_name_array[E_hook] = L"炸弹";
            entity_name_array[E_bubble] = L"泡泡";
            
            if(button_index == edit_add_entity)
            {
                _swprintf(temp_name , L"%s (%s)" , name , entity_name_array[current_edit_entity_type]);
                name = temp_name;
            }
            
            button_list_index++;
            wchar_t name_buffer[64] = {};
            _swprintf(name_buffer , L"%s : %d" , name , button_list_index);
            
            if(key_pressed(KEY_ZERO + button_list_index))
            {
            ...
            if(button_index == edit_add_entity)
            {
                DrawingMenu entity_menu = side_list_menu;
                entity_menu.menu_type = GMT_to_the_right;
                draw_menu_button_W(&side_list_menu , 0);
                
                if(draw_menu_button_W_EX(&entity_menu , name_buffer , YELLOW ,  current_edit_type == button_index))
                {
                    current_edit_type = button_index;
                }
                
                for(int entity_index = 0; entity_index < E_count; entity_index++)
                {
                    if(draw_menu_button_W_EX(&entity_menu , entity_name_array[entity_index] , YELLOW , current_edit_entity_type == entity_index))
                    {
                        current_edit_entity_type = entity_index;
                    }
                }
            }
            else
            {
                if(draw_menu_button_W_EX(&side_list_menu , name_buffer , YELLOW ,  current_edit_type == button_index))
                {
                    current_edit_type = button_index;
                }
            }
            ...
            if(button_index == edit_camera_offset)
            {
                if(last_clicked_box)
                {
                    ...
                    camera_offset[count++] = (Vector3){GRID_SIZE * 0 , -GRID_SIZE * 10, -GRID_SIZE * 6};
        ...
        if(draw_menu_button_W(&top_right_bar , show_bounding_box? L"关闭边界框" : L"显示边界框"))
        {
        ...
        if(!store_multiple_frame_collision)
        {
            ...
            if(frame_collision_buffer.count > 0)
            {
                ...
                _swprintf(frame_buffer_string , L"碰撞：%d (%d)" , current_frame_collision_index , frame_collision_buffer.count);
                ...
                if(show_frame_drop_down)
                {
                    for(int frame_index = 0 ; frame_index < frame_collision_buffer.count ; frame_index++)
                    {
                        ...
                        if((frame_collision.slice_end - frame_collision.slice_start) > 0)
                        {
                            ...
                            _swprintf(frame_string , L"帧 %d, %d:(%d->%d)" , frame_collision.update_index , frame_index , frame_collision.slice_start + 1 , frame_collision.slice_end);
...
internal bool function drag_to_rotate(Vector3 decl position , Quaternion * decl rotation_to_modifying , bool * decl hovering)
{
    ...
    if(rotating_index != -1)
    {
        float decl angle = atan2(angle_xy.y , angle_xy.x);
        float decl delta_angle = angle - start_angle;
...
internal bool function drag_to_move(Vector3 * position_to_modify , bool * hovering)
{
    ...
    if(!dragging_origin)
    {
        if(box_collision_ray(mouse_ray_3D.position , mouse_ray_3D.direction , right_box , 0 , 0 , 0)) 
        {
            if(mouse_pressed(MOUSE_BUTTON_LEFT))
            {
        ...
        if(box_collision_ray(mouse_ray_3D.position , mouse_ray_3D.direction , up_box , 0 , 0 , 0)) 
        {
            if(mouse_pressed(MOUSE_BUTTON_LEFT))
            {
                dragging_origin = true;
        ...
        if(box_collision_ray(mouse_ray_3D.position , mouse_ray_3D.direction , forward_box , 0 , 0 , 0)) 
        {
            if(mouse_pressed(MOUSE_BUTTON_LEFT))
            {
                dragging_origin = true;
    ...
    if(dragging_origin)
    {
        ...
        Vector3 position_in_cell = position_to_grid(origin_position , drag_grid_size);
...
internal bool function bone_rotate_and_check(Bone * final_bone_array_copy, int single_bone_index , Clip * clip_to_assign)
{
    ...
    bool hovering = false;
    for(int decl axis_index = 0; axis_index < R_count; axis_index++)
    {
    ...
    local_persist Vector3 decl previous_drag_point = {};
    local_persist int decl dragging_axis = -1;
    ...
    int decl closest_axis_index = -1;
    
    for(int decl axis_index = 0; axis_index < R_count; axis_index++)
    {
        if(!rotation_rect_collision[axis_index].hit) continue;
        if(rotation_rect_collision[axis_index].distance > closest_collision.distance) continue;
    ...
    if(closest_axis_index != -1)
    {
        if(mouse_pressed(MOUSE_BUTTON_LEFT))
        {
            dragging_axis = closest_axis_index;
        }
    }
    ...
    {
        Vector3 decl hit_point = mouse_on_plane(selected_bone->position);
        Rect decl axis_rect = rotation_rect[dragging_axis];
...
//what matrix old me????
internal void function bone_selection_and_edit_bone_state( int current_frame_index)
{
    if(key_pressed(KEY_E))
    {
    ...
    //this thing is messy
    local_persist bool decl redrag_selection = true;
    ...
    bool decl curve_visualization = true;
    ...
    if(curve_visualization)
    {
        ...
        local_persist bool decl interacting_with_bezier_curve = false;
        local_persist bool decl modify_key_frame_position_only = false;
        local_persist bool decl selected_previous_bezier_point = false;
        local_persist KeyFrame * decl selected_bezier_key_frame = 0;
        local_persist KeyFrame * decl selected_bezier_other_key_frame = 0;
        local_persist Vector3 decl selected_bezier_base_bone_offset = {};
        local_persist Quaternion decl selected_bezier_base_bone_rotation = {};
        //local_persist _Bone * SelectedBezierBone = 0;
        
        for(int decl select_stack_index = 0 ; select_stack_index < editor->selected_bone_count ; select_stack_index++)
        {
            BoneSelection * decl bone_selection = editor->selected_bone_stack + select_stack_index;
            ClipBone * decl selected_clip_bone = clip_bone_stack + bone_selection->clip_bone_stack_index;
            Clip * decl selected_clip = clip_array[bone_selection->clip_index];
            
            int decl selected_bone_index = bone_selection->bone_index;
            
            Bone * decl selected_final_bone = selected_clip_bone->final_bone_pose + selected_bone_index;
            Bone * decl selected_offset_bone = selected_clip_bone->bone_pose_offset + selected_bone_index;
            
            int decl previous_key_frame_index = -1;
            hash_table_iterate_reverse(decl key_frame_index , selected_bone_index , &selected_clip->key_frame_hash_table_by_bone)
            {
                previous_key_frame_index = key_frame_index;
                break;
            }
            
            KeyFrame * decl previous_key_frame = all_key_frame_buffer.data + previous_key_frame_index;
            
			Color decl key_frame_line_color = Fade(WHITE , 0.2f);
            ...
            //draw_bezier_path
            hash_table_iterate(decl key_frame_index , selected_bone_index , &selected_clip->key_frame_hash_table_by_bone)
            {
                KeyFrame * decl key_frame = all_key_frame_buffer.data + key_frame_index;
                if(key_frame->bone_index != selected_bone_index) continue;
                
                Quaternion decl base_bone_rotation = selected_final_bone->rotation;
                Vector3 decl base_bone_offset =  Vector3RotateByQuaternion(selected_offset_bone->state.local_position , base_bone_rotation);
				Vector3 decl base_bone_position = Vector3Subtract( selected_final_bone->position , base_bone_offset);
                
                //DrawArrowRay(SelectedFinalBone->position , Vector3Negate(BaseBoneOffset) , RED);
                
                Vector3 decl previous_key_frame_offset_position = Vector3RotateByQuaternion(previous_key_frame->bone_state.local_position , base_bone_rotation);
                Vector3 decl previous_key_frame_position = Vector3Add( base_bone_position , previous_key_frame_offset_position);
                
				Vector3 decl key_frame_offset_position = Vector3RotateByQuaternion(key_frame->bone_state.local_position , base_bone_rotation);
                Vector3 decl key_frame_position = Vector3Add( base_bone_position , key_frame_offset_position);
                ...
                Vector3 decl line_direction = Vector3Subtract(key_frame_position , previous_key_frame_position);
                line_direction = Vector3Scale(Vector3Normalize(line_direction),0.1f);
                
                Vector3 decl previous_key_frame_bezier_point = Vector3Add(line_direction , Vector3RotateByQuaternion(previous_key_frame->next_bezier_offset , base_bone_rotation));
                previous_key_frame_bezier_point = Vector3Add(previous_key_frame_bezier_point , previous_key_frame_position);
                
                Vector3 decl key_frame_bezier_point = Vector3Add(Vector3Negate(line_direction) , Vector3RotateByQuaternion(key_frame->previous_bezier_offset , base_bone_rotation));
                ...
                Rect decl previous_bezier_point_rect = get_billboard_rect(previous_key_frame_bezier_point , 0.1);
                Rect decl bezier_point_rect = get_billboard_rect(key_frame_bezier_point , 0.1);
                
                Color decl previous_bezier_point_color = Fade(BLUE , 0.5f);
                Color decl bezier_point_color = Fade(RED , 0.5f);
                Color decl key_frame_point_color = Fade(YELLOW , 0.5f);
                ...
                int decl frame_count = get_key_frame_count(previous_key_frame , key_frame ,editor->start_frame_index  , editor->timeline_frame_length );
                
                for(int decl frame_index = 0 ; frame_index < frame_count ; frame_index++)
                {
                    float decl bezier_start_time = ((float)frame_index) / ((float)frame_count);
                    float decl bezier_end_time = ((float)(frame_index+1)) / ((float)frame_count);
                    
                    Vector3 decl bezier_start_point = vector3_cubic_bezier(previous_key_frame_position , previous_key_frame_bezier_point , key_frame_bezier_point, key_frame_position  , bezier_start_time);
                    Vector3 decl bezier_end_point = vector3_cubic_bezier(previous_key_frame_position , previous_key_frame_bezier_point , key_frame_bezier_point, key_frame_position  , bezier_end_time);
                ...
                Rect decl key_frame_rect = get_billboard_rect(key_frame_position , 0.1 );
        ...
        {
            local_persist Vector3 decl dragging_plane_point = {};
            local_persist Vector3 decl previous_dragging_point = {};
            ...
            if(modify_key_frame_position_only)
            {
                ...
                Vector3 decl dragging_point = get_collision_quad_3D(get_billboard_quad(dragging_plane_point , 100000,100000)).point;
                
                Vector3 decl new_key_frame_offset = Vector3Subtract(dragging_point , previous_dragging_point);
            ...
            {
                Vector3 decl start_point = {};
                Vector3 decl end_point = {};
                Vector3 decl bezier_offset = {};
                ...
                Vector3 decl direction = Vector3Subtract(end_point , start_point);
                direction = Vector3Scale(Vector3Normalize(direction),0.1f);
                
                Vector3 decl bezier_direction = Vector3Add(direction , bezier_offset);
                //BezierDirection = Vector3RotateByQuaternion(BezierDirection , SelectedBezierBaseBonerotation);
                
                Vector3 decl bezier_start = Vector3Add(Vector3RotateByQuaternion(selected_bezier_key_frame->bone_state.local_position , selected_bezier_base_bone_rotation) , selected_bezier_base_bone_offset);
                Vector3 decl bezier_end = Vector3Add(bezier_start , bezier_direction );
                
                Rect decl bezier_end_rect = get_billboard_rect(bezier_end , 0.1);
                ...
                //TODO : it just snap to the point instead adding offset
                Vector3 decl dragging_point = get_collision_quad_3D(get_billboard_quad(dragging_plane_point , 100000,100000)).point;
                Vector3 decl new_bezier_end_point = Vector3Add(bezier_end ,Vector3Subtract(dragging_point , previous_dragging_point));
                previous_dragging_point = dragging_point;
                Vector3 decl new_bezier_offset = Vector3Subtract(new_bezier_end_point , bezier_start);
    ...
    //hated it
    //terrible control
    //function bone_single_selection_and_drag_multiple
    local_persist bool decl dragging_bone_position = false;
    local_persist Vector3 decl previous_drag_point ={};
	local_persist Vector3 decl drag_plane_position = {};
    ...
    for(int decl clip_bone_stack_index = 0 ; clip_bone_stack_index  < clip_bone_stack_count ; clip_bone_stack_index++)
    {
        ClipBone * decl clip_bone = clip_bone_stack + clip_bone_stack_index;
        
        for(int decl bone_index = 0 ; bone_index < selected_model->bone_buffer.count ; bone_index++)
        {
            clip_bone->hovered_bone[bone_index] = false;
            
            Bone * decl final_bone = clip_bone->final_bone_pose + bone_index;
			Vector3 decl bone_interact_point ={};
            ...
            else
            {
                ...
                Vector3 decl bone_position_end = Vector3RotateByQuaternion(final_bone->state.end_point_offset , final_bone->rotation);
                bone_position_end = Vector3Add(bone_position_end , final_bone->position);
                
                bone_interact_point = bone_position_end;
            }
            
            Vector3 decl bone_screen_point = transform_vector(bone_interact_point , world_3D_to_screen_matrix);
            
            Rect decl bone_screen_rect = get_rect();
            bone_screen_rect.position = bone_screen_point;
            bone_screen_rect.size = (Vector2){40 ,40};
            
            Vector3 decl hit_point = mouse_on_plane(bone_interact_point);
            Vector2 decl current_mouse_position = app_data->mouse_position;
            
            //function bone_click_to_select
            if(check_collision_rect(bone_screen_rect , current_mouse_position))
            {
                ...
                if(mouse_pressed(MOUSE_BUTTON_LEFT))
                {
                    if(!clip_bone->selected_bone[bone_index])
                    {
                        ...
                        {
                            BoneSelection * decl NewBoneSelection = editor->selected_bone_stack + editor->selected_bone_count++;
                        ...
                        else
                        {
                            ...
                            {
                                ClipBone * decl ClipBoneToUnSelect = clip_bone_stack + StackIndex;
    ...
    //function bone_apply_drag_offset_to_existing_key_frame
    if(dragging_bone_position)
    {
        Vector3 decl drag_point = mouse_on_plane(drag_plane_position);
        Vector3 decl plane_offset= drag_point;
        plane_offset = Vector3Subtract(plane_offset, previous_drag_point);
        
        previous_drag_point = drag_point;
        
        for(int decl selected_bone_stack_index = 0 ; selected_bone_stack_index < editor->selected_bone_count ; selected_bone_stack_index++)
        {
            BoneSelection * decl dragging_bone_selection = editor->selected_bone_stack + selected_bone_stack_index;
            
            int decl dragging_bone_index = dragging_bone_selection->bone_index;
            ClipBone * decl dragging_clip_bone = clip_bone_stack + dragging_bone_selection->clip_bone_stack_index;
            Bone * decl dragging_bone = dragging_clip_bone->final_bone_pose + dragging_bone_index;
            
            //it just feel right to do it
            //also the world rotation is compute after
            //something could go wrong
            Quaternion decl revert_rotation = QuaternionMultiply(dragging_bone->rotation , QuaternionInvert(dragging_bone->state.local_rotation));
            ...
            int decl assign_key_frame_count = 0;
            
            if(dragging_bone->free_position)
            {
                if(!editor->playing)
                {
                    Clip * decl current_clip = clip_array[dragging_clip_bone->clip_index];
                    
                    hash_table_iterate(decl key_frame_index , GetKeyFrameHash(editor->current_frame_at_timeline , dragging_bone_index) , &current_clip->key_frame_hash_table)
                    {
                        KeyFrame * decl key_frame = all_key_frame_buffer.data + key_frame_index;
    ...
    {
        //function bone_rotate_single
        pruning_3D_line = true;
        change_matrix(world_3D_matrix);
        if(editor->selected_bone_count == 1)
        {
            BoneSelection * decl single_bone_selection = editor->selected_bone_stack;
            
            int decl single_bone_index = single_bone_selection->bone_index;
            Bone * decl final_bone_array_copy = clip_bone_stack[single_bone_selection->clip_bone_stack_index].final_bone_pose;
            Bone * decl selected_bone =  final_bone_array_copy + single_bone_index;
            Quaternion * decl rotation_to_modify = &selected_bone->state.local_rotation;
            Vector3 decl rotation_position = selected_bone->position;
            if(editor_type == edit_base_pose) rotation_to_modify = &selected_model->bone_buffer.data[single_bone_index].state.local_rotation;
            bool decl hovering = false;
            
            Quaternion decl rotation = (*rotation_to_modify);
            printf("%f %f %f %f\n" , rotation.x , rotation.y , rotation.z , rotation.w);
            drag_to_rotate(rotation_position , rotation_to_modify , &hovering);
            rotation = (*rotation_to_modify);
            printf("%f %f %f %f\n" , rotation.x , rotation.y , rotation.z , rotation.w);
            ...
            if(!editor->playing)
            {
                if(editor->selected_clip_index != -1)
                {
                    if(editor_type == edit_animation)
                    {
                        int decl key_frame_count = 0;
                        hash_table_iterate(decl key_frame_index , GetKeyFrameHash(editor->current_frame_at_timeline , single_bone_index) , &clip_array[editor->selected_clip_index]->key_frame_hash_table)
                        {
                            KeyFrame * key_frame = all_key_frame_buffer.data + key_frame_index;
                            if(key_frame->bone_index != single_bone_index) continue;
                            if(key_frame->frame_index != editor->current_frame_at_timeline) continue;
                            
                            key_frame->bone_state.local_rotation = selected_bone->state.local_rotation;
            ...
            else if(mouse_pressed_no_check(MOUSE_BUTTON_LEFT))
            {
                editor->selected_bone_count = 0;
            }
        }
    ...
    //function bone_multi_selection
    local_persist Vector3 decl mouse_start_drag_position = {};
    Vector2 decl mouse_screen_position = app_data->mouse_position;
    ...
    local_persist bool decl dragging_selection = false;
    
    if(redrag_selection)
    {
        Vector3 decl current_mouse_drag_position = {mouse_screen_position.x , mouse_screen_position.y , 0};
        
        Rect decl mouse_drag_rect = get_rect();
        ...
        for(int decl clip_bone_stack_index = 0 ; clip_bone_stack_index < clip_bone_stack_count ; clip_bone_stack_index++)
        {
            ClipBone * decl current_clipBone = clip_bone_stack + clip_bone_stack_index;
            
            for(int decl bone_index = 0 ; bone_index < selected_model->bone_buffer.count ; bone_index++)
            {
                Bone * decl current_bone = current_clipBone->final_bone_pose + bone_index;
				Vector3 decl bone_screen_point = {};
                ...
                else
                {
					...
					Vector3 decl bone_position_end = Vector3RotateByQuaternion(current_bone->state.end_point_offset , current_bone->rotation);
                    ...
                    if(dragging_selection)
                    {
                        ...
                        if(mouse_released(MOUSE_BUTTON_LEFT))
                        {
                            if(!current_clipBone->selected_bone[bone_index])
                            {
                                ...
                                BoneSelection * decl new_selection = editor->selected_bone_stack + editor->selected_bone_count++;
    ...
    for(int decl clip_bone_stack_index = 0 ; clip_bone_stack_index < clip_bone_stack_count ; clip_bone_stack_index++)
    {
        ClipBone * decl current_clip_bone = clip_bone_stack + clip_bone_stack_index;
        
        for(int decl bone_index = 0 ; bone_index < selected_model->bone_buffer.count ; bone_index++)
        {
            Bone * decl bone = current_clip_bone->final_bone_pose + bone_index;
            ...
            else
            {
                ...
                Vector3 decl bone_position_end = Vector3RotateByQuaternion(bone->state.end_point_offset , bone->rotation);
...
internal void function iterate_bone_structure(Bone * bone_array , Bone * root_bone)
{
...
internal void function update_bone_structure( Bone * bone_array)
{
...
internal void function bone_IK_update_EX( Bone * bone_array , Bone * base_pose_bone_array , Bone * target_bone , Bone * pole_bone , Bone * IK_bone , int iteration_count , int bone_chain_max_length)
{
    Bone * bone_stack[256] = {};
    int bone_stack_count = 0;
...
internal void function bone_IK_update(Bone * bone_array , Bone * base_pose_bone_array , int target_bone_index , int pole_bone_index , int IK_bone_index , int iteration_count , int bone_chain_max_length)
{
...
internal void function get_face_from_face_index(Box box ,int box_face_index , Vector3 * offset , Vector3 * direction)
{
    Vector3 face_offset = {};
    ...
    if(offset) (*offset) = face_offset;
    if(direction) (*direction) = face_direction;
}

internal void function stretch_box(Box * box_to_stretch)
{
    Box box = (*box_to_stretch);
    
    local_persist int box_face_index_to_stretch = 0;
    
    try_stretch_box = false;
    ...
    if(!stretching_box)
    {
        if(box_collision_ray(mouse_ray_3D.position , mouse_ray_3D.direction , box , &box_face_index , &box_hit_time , 0))
        {
            try_stretch_box = true;
            Vector3 face_offset = {};
            Vector3 face_direction = {};
            
            get_face_from_face_index(box ,box_face_index , &face_offset , &face_direction);
            
            Vector3 arrow_start = Vector3Add(mouse_ray_3D.position , Vector3Scale(mouse_ray_3D.direction , box_hit_time));
...
internal void function add_to_ray_collection(BoxRayDataBuffer * buffer , BoxRayData new_box_ray_data)
{
    if(buffer_full(*buffer)) reallocate_buffer(buffer , AT_frame);
    buffer->data[buffer->count++] = new_box_ray_data;
    
    for(int buffer_index = buffer->count - 1; buffer_index > 0; buffer_index--)
    {
        int next_buffer_index = buffer_index - 1;
        
        BoxRayData * box_ray_data = buffer->data + buffer_index;
        BoxRayData * next_box_ray_data = buffer->data + next_buffer_index;
        
        if(box_ray_data->hit_time < next_box_ray_data->hit_time)
        {
            BoxRayData temp = (*box_ray_data);
            (*box_ray_data) = (*next_box_ray_data);
            (*next_box_ray_data) = temp;
        }
        else
        {
            break;
...
internal void function edit_box(Vector3 mouse_on_grid , Vector3 intersect_point)
{
    local_persist bool tilting_box = false;
    local_persist int tilting_face_index = 0;
    local_persist Vector3 tilt_offset = {};
    local_persist Vector3 tilt_start_position = {};
    local_persist Vector3 previous_tilt_point = {};
    ...
    if(last_clicked_box)
    {
        ...
        {
            Vector3 box_corner = box_to_point((*box))[bv_G];
            Vector3 box_corner_to_position = Vector3Subtract(box->position , box_corner);
            drag_to_move(&box_corner , 0);
            box->position = Vector3Add(box_corner , box_corner_to_position);
        }
        
        if(current_edit_type == edit_rotate)
        {
            drag_to_rotate(box->position , &box->rotation , 0);
        }
    }
    ...
    if(last_clicked_entity)
    {
        if(current_edit_type == edit_move)
        {
            Box entity_box = last_clicked_entity->box;
            entity_box.position = last_clicked_entity->position;
            
            Vector3 box_corner_position = box_to_point(entity_box)[bv_G];
            Vector3 box_corner_offset = Vector3Subtract(last_clicked_entity->position , box_corner_position);
            
            if(drag_to_move(&box_corner_position , 0))
            {
                last_clicked_entity->position = Vector3Add(box_corner_position , box_corner_offset);
            }
        }
    }
    
    bool try_tilt_box = false;
    
    if(current_edit_type == edit_tilt)
    {
        if(last_clicked_box)
        {
            Box box = (*last_clicked_box);
            ...
            if(!tilting_box)
            {
                ...
                if(box_collision_ray(mouse_ray_3D.position , mouse_ray_3D.direction , box , &box_face_index , &box_hit_time , 0))
                {
                    ...
                    {
                        tilting_box = true;
                        tilting_face_index = box_face_index;
                        previous_tilt_point = Vector3Add(mouse_ray_3D.position , Vector3Scale(mouse_ray_3D.direction , box_hit_time));
                        tilt_offset = (Vector3){};
                        tilt_start_position = box.position;
                        
                        switch(tilting_face_index)
                        {
                            case face_front:
                            {
                                tilt_offset.y = box.front_top;
                                tilt_offset.x = box.front_right;
                            }
                            break;
                            ...
                            case face_left:
                            {
                                tilt_offset.y = box.right_top * 2;
                                tilt_offset.z = box.right_front * 2;
                            }
                            break;
                            ...
                            case face_bottom:
                            {
                                tilt_offset.x = box.top_right * 2;
                                tilt_offset.z = box.top_front * 2;
                            }
                            break;
                        }
                        
                        tilt_start_position.x -= tilt_offset.x;
                        tilt_start_position.y -= tilt_offset.y;
                        tilt_start_position.z -= tilt_offset.z;
                        
                        tilt_offset = Vector3Scale(tilt_offset , 2);
                    }
                }
            }
            ...
            if(tilting_box)
            {
                Quad box_faces[face_count] = {};
                get_box_face(box_faces , box);
                Quad quad = box_faces[tilting_face_index];
                ...
                Vector3 end = Vector3Add(mouse_ray_3D.position , mouse_ray_3D.direction);
                float intersect_time = get_line_intersect_with_plane_time(mouse_ray_3D.position , end , normal , a);
                Vector3 point = Vector3Lerp(mouse_ray_3D.position , end , intersect_time);
                
                Vector3 offset = Vector3Subtract(point , previous_tilt_point);
                tilt_offset = Vector3Add(tilt_offset , offset);
                previous_tilt_point = point;
                
                for(int vertex_index = 0; vertex_index < quad_vertex_count; vertex_index++)
                {
                    quad.vertex_position[vertex_index] = Vector3Add(quad.vertex_position[vertex_index] , tilt_offset);
                }
                
                Vector3 tilt_offset_in_grid = position_to_cell(tilt_offset , UNIT_SIZE);
                
                if(key_pressing(KEY_LEFT_SHIFT))
                {
                    if(fabs(tilt_offset_in_grid.x) > fabs(tilt_offset_in_grid.y))
                    {
                        if(fabs(tilt_offset_in_grid.x) > fabs(tilt_offset_in_grid.z))
                        {
                            tilt_offset_in_grid.y = 0;
                            tilt_offset_in_grid.z = 0;
                        }
                        else
                        {
                            tilt_offset_in_grid.x = 0;
                            tilt_offset_in_grid.y = 0;
                        }
                    }
                    else if(fabs(tilt_offset_in_grid.y) > fabs(tilt_offset_in_grid.z))
                    {
                        tilt_offset_in_grid.x = 0;
                        tilt_offset_in_grid.z = 0;
                    }
                ...
                if(tilting_face_index == face_front)
                {
                    box.front_right = tilt_offset_in_grid.x * 0.5f;
                    box.front_top = tilt_offset_in_grid.y * 0.5f;
                    
                    box.position.x = tilt_start_position.x + tilt_offset_in_grid.x * 0.5f;
                    box.position.y = tilt_start_position.y + tilt_offset_in_grid.y * 0.5f;
                }
                else if(tilting_face_index == face_back)
                {
                    box.front_right = -tilt_offset_in_grid.x * 0.5f;
                    box.front_top = -tilt_offset_in_grid.y * 0.5f;
                ...
                else if(tilting_face_index == face_right)
                {
                    box.right_top = tilt_offset_in_grid.y * 0.5f;
                    box.right_front = tilt_offset_in_grid.z * 0.5f;
                    
                    box.position.y += tilt_offset_in_grid.y * 0.5f;
                    box.position.z += tilt_offset_in_grid.z * 0.5f;
                }
                else if(tilting_face_index == face_left)
                {
                    box.right_top = -tilt_offset_in_grid.y * 0.5f;
                    box.right_front = -tilt_offset_in_grid.z * 0.5f;
                ...
                else if(tilting_face_index == face_top)
                {
                    box.top_right = tilt_offset_in_grid.x * 0.5f;
                    box.top_front = tilt_offset_in_grid.z * 0.5f;
                    
                    box.position.x += tilt_offset_in_grid.x * 0.5f;
                    box.position.z += tilt_offset_in_grid.z * 0.5f;
                }
                else if(tilting_face_index == face_bottom)
                {
                    box.top_right = -tilt_offset_in_grid.x * 0.5f;
                    box.top_front = -tilt_offset_in_grid.z * 0.5f;
    ...
    {
        if(last_clicked_box) stretch_box(last_clicked_box);
        if(last_clicked_entity) 
        {
            stretch_box(&last_clicked_entity->box);
            last_clicked_entity->position = last_clicked_entity->box.position;
        }
    }
    ...
    local_persist bool drag_on_other_box = false;
    local_persist Vector3 drag_plane_normal = {};
    local_persist Vector3 drag_plane_origin = {};
    ...
    if(drag_and_make_box)
    {
        ...
        Vector3 dragging_point = mouse_on_grid;
        
        if(drag_on_other_box)
        {
            float intersect_time = get_line_intersect_with_plane_time(mouse_ray_3D.position , Vector3Add(mouse_ray_3D.position , mouse_ray_3D.direction) , drag_plane_normal , drag_plane_origin);
            dragging_point = Vector3Add(mouse_ray_3D.position , Vector3Scale(mouse_ray_3D.direction , intersect_time));
            dragging_point = position_to_cell(dragging_point , GRID_SIZE);
        }
        
        dragging_box.size = Vector3Subtract(dragging_point , box_start_point);
        ...
        dragging_box.position = Vector3Lerp(dragging_point , box_start_point , 0.5f);
    ...
    if(current_edit_type == edit_add_entity)
    {
        ...
        if(mouse_pressed(MOUSE_BUTTON_LEFT))
        {
            if(array_full(&entity_layout_array))
            {
                reallocate_array(&entity_layout_array , AT_temp);
                reallocate_buffer(&entity_layout_buffer , AT_temp);
            }
            
            int new_entity_index = add_to_array(&entity_layout_array);
            
            Entity * new_entity = entity_layout_buffer.data + new_entity_index;
            int new_generation_index = new_entity->generation_index + 1;
            (*new_entity) = (Entity){};
            
            //new_entity->player_index = -1;
            new_entity->type = current_edit_entity_type;
            new_entity->entity_index = new_entity_index;
            new_entity->generation_index = new_generation_index;
            new_entity->position = mouse_on_grid_centre;
            new_entity->box = get_box();
            new_entity->box.size = entity_box_size;
            new_entity->position = new_entity->position;
        }
    }
    
    bool select_box = true;
    
    if(try_stretch_box) select_box = false;
    if(try_tilt_box) select_box = false;
    if(stretching_box) select_box = false;
    if(tilting_box) select_box = false;
    if(current_edit_type == edit_add_entity) select_box = false;
    
    bool try_select_entity = true;
    
    if(select_box)
    {
        BoxRayDataBuffer box_ray_data_buffer = {};
        
        allocate_buffer(&box_ray_data_buffer , BoxRayData , 8 , AT_frame);
        
        if(current_map_edit_type == MET_box)
        {
            array_foreach(entity_index , &entity_layout_array)
            {
                Entity * entity = entity_layout_buffer.data + entity_index;
                Box entity_box = entity->box;
                
                float hit_time = 0;
                int box_face_index = 0;
                
                if(box_collision_ray(mouse_ray_3D.position , mouse_ray_3D.direction , entity_box , &box_face_index , &hit_time , 0))
                {
                    BoxRayData new_box_ray_data = {};
                    new_box_ray_data.is_entity = true;
                    new_box_ray_data.is_box = false;
                    new_box_ray_data.data_index = entity_index;
                    new_box_ray_data.hit_time = hit_time;
                    new_box_ray_data.box_face_index = box_face_index;
                    
                    add_to_ray_collection(&box_ray_data_buffer , new_box_ray_data);
                }
            }
        }
        ...
        array_foreach(box_index , current_box_array)
        {
            ...
            {
                BoxRayData new_box_ray_data = {};
                new_box_ray_data.is_entity = false;
                new_box_ray_data.is_box = true;
                new_box_ray_data.data_index = box_index;
                new_box_ray_data.hit_time = box_hit_time;
                new_box_ray_data.box_face_index = box_face_index;
        ...
        bool closest_is_entity = false;
        bool closest_is_box = false;
        
        bool last_is_box = false;
        bool last_is_entity = false;
        void * last_in_ray = 0;
        
        for(int buffer_index = 0; buffer_index < box_ray_data_buffer.count; buffer_index++)
        {
            box_collided = true;
            
            BoxRayData box_ray_data = box_ray_data_buffer.data[buffer_index];
            
            int data_index = box_ray_data.data_index;
            float box_hit_time = box_ray_data.hit_time;
            int box_face_index = box_ray_data.box_face_index;
            bool is_entity = box_ray_data.is_entity;
            bool is_box = box_ray_data.is_box;
            
            Box * box = 0;
            Entity * entity = 0;
            
            if(is_box) box = current_box_buffer->data + data_index;
            if(is_entity) entity = entity_layout_buffer.data + data_index;
            
            if(!last_in_ray) 
            {
                if(is_box) last_in_ray = box;
                if(is_entity) last_in_ray = entity;
                
                last_is_box = is_box;
                last_is_entity = is_entity;
            }
            
            if(current_edit_type != edit_drag_new_box)
            {
                if(is_box) draw_box_line((*box) , Fade(WHITE , 0.2f) , 2);
                if(is_entity) draw_box_line(entity->box , Fade(WHITE , 0.2f) , 2);
            }
            
            if(closest_box_hit_time > box_hit_time)
            {
                closest_box_hit_time = box_hit_time;
                if(is_box) closest = box;
                if(is_entity) closest = entity;
                closest_is_box = is_box;
                closest_is_entity = is_entity;
                closest_box_face_index = box_face_index;
            }
            
            bool same_as_last_clicked = false;
            
            if(last_clicked_entity)
            {
                if(last_clicked_entity == entity)
                {
                    same_as_last_clicked = true;
                }
            }
            
            if(last_clicked_box)
            {
                if(last_clicked_box == box)
                {
                    same_as_last_clicked = true;
                }
            }
            
            if(same_as_last_clicked)
            {
                closest_box_hit_time = FLT_MAX;
                closest = last_in_ray;
                closest_is_box = last_is_box;
                closest_is_entity = last_is_entity;
                closest_box_face_index = box_face_index;
            }
        }
        ...
        if(current_edit_type != edit_drag_new_box)
        {
            if(box_collided)
            {
                if(mouse_pressed(MOUSE_BUTTON_LEFT))
                {
                    ...
                    if(closest_is_box) last_clicked_box = closest;
                    if(closest_is_entity) last_clicked_entity = closest;
                }
            }
        }
        ...
        if(current_edit_type == edit_drag_new_box)
        {
            if(mouse_pressed(MOUSE_BUTTON_LEFT))
            {
                ...
                array_foreach(box_index , &box_in_map_array)
                {
                    ...
                    float hit_time = 0;
                    Vector3 hit_normal = {};
                    if(box_collision_ray(mouse_ray_3D.position , mouse_ray_3D.direction , box , 0 , &hit_time , &hit_normal))
                    {
                        if(closest_hit_time > hit_time)
                        {
                            closest_hit_time = hit_time;
                            Vector3 hit_point = Vector3Add(mouse_ray_3D.position , Vector3Scale(mouse_ray_3D.direction , hit_time));
                            drag_on_other_box = true;
                            drag_plane_normal = hit_normal;
                            drag_plane_origin = hit_point;
                            box_start_point = position_to_cell(hit_point , GRID_SIZE);
                        }
                    }
                }
            }
...
internal void function edit_map(Vector3 origin)
{
    Vector3 camera_direction = Vector3Subtract(world_camera.target , world_camera.position);
    ...
    if(editor_type == edit_world)
    {
        ...
        if(current_map_edit_type == MET_box)
        {
            array_foreach(array_index , &box_in_map_array)
            {
                ...
                float hit_time = 0;
                int hit_face = 0;
                if(box_collision_ray(mouse_ray_3D.position , mouse_ray_3D.direction , box , &hit_face , &hit_time , 0))
                {
                    if(closest_box_hit_time > hit_time)
                    {
                        closest_box_hit_time = hit_time;
                        get_face_from_face_index(box , hit_face , 0 , &grid_normal);
                        grid_normal = Vector3Normalize(grid_normal);
                        intersect_point = Vector3Add(mouse_ray_3D.position , Vector3Scale(mouse_ray_3D.direction , hit_time));
                    }
                }
            }
        }
        ...
        mouse_on_grid_centre = intersect_point;
        mouse_on_grid_centre = Vector3Subtract(mouse_on_grid_centre , origin);
        
        mouse_on_grid_centre = Vector3Add(mouse_on_grid_centre , Vector3Scale(grid_normal , -mouse_grid_size * 0.5));
        ...
        bool flipped = false;
        if(Vector3DotProduct(Vector3Subtract(world_camera.position, intersect_point) , grid_normal) > 0) flipped = true;
        
        if(flipped)
        {
            mouse_on_grid_centre.x += mouse_grid_size * grid_normal.x;
            mouse_on_grid_centre.y += mouse_grid_size * grid_normal.y;
            mouse_on_grid_centre.z += mouse_grid_size * grid_normal.z;
        }
        ...
        if(within_viewport)
        {
                ...
                if(current_edit_type == edit_camera_offset)
                {
                    if(edit_camera_offset)
                    {
                        if(last_clicked_box)
                        {
                            ...
                            camera_position.x += GRID_SIZE * 0.5f;
                            camera_position.y += GRID_SIZE * 0.5f;
                            camera_position.z += GRID_SIZE * 0.5f;
            ...
            if(current_edit_type == edit_player_position)
            {
                Box player_box = get_box();
                player_box.position = player_spawn_point;
                player_box.size = player_box_size;
                draw_box_line(player_box , Fade(WHITE , 0.5) , 5 );
                player_box.size = (Vector3){GRID_SIZE , GRID_SIZE , GRID_SIZE};
                Vector3 corner = box_to_point(player_box)[bv_G];
                Vector3 offset = Vector3Subtract(player_spawn_point , corner);
                
                drag_to_move(&corner , 0);
                player_spawn_point = Vector3Add(corner , offset);
            }
...
internal void function draw_origin_grid(Vector3 origin)
{
#if 1
...
internal void function get_bone_from_existing_key_frame(Bone * bone_array , Clip * clip , int target_frame , int target_start_frame , int target_frame_count)
{
...
internal void function add_bone_state(Bone * bone_array , Bone * add_bone_array , int bone_index)
{
...
internal void function add_multiple_bone_state(Bone * bone_array , Bone * add_bone_array , int bone_array_count)
{
...
internal Bone * function get_multiple_bone(int bone_array_count)
{
    Bone * temp_bone = allocate_memory_type(Bone ,bone_array_count , AT_frame);
...
internal Bone * function get_bone_pose_offset_from_clip( int clip_index , int target_frame , int target_frame_start, int target_frame_length)
{
...
internal float function get_corner_weight(float vertical , float horizontal)
{
...
internal void function draw_collision_visual(CollisionVisual visual , Vector3 velocity)
{
    draw_simplex_triangle( visual.a , visual.b , visual.c);
    draw_arrow_line(visual.closest_point , visual.ray_end  , Fade(PINK , 0.5f));
    if(visual.collided) draw_arrow_ray( visual.collision_point , visual.collision_normal , RED );
    draw_arrow_ray(visual.offset , visual.direction , Fade(BLUE , 0.5f));
    
    ShapeUnion shape_a = visual.shape_a;
    ShapeUnion shape_b = visual.shape_b;
    
    if(shape_a.type == ST_box) 
    {
        draw_box_line(shape_a.box , WHITE , 5);
        shape_a.box.position = Vector3Add(shape_a.box.position , velocity);
        draw_box_line(shape_a.box , WHITE , 5);
    }
    
    if(shape_a.type == ST_quad) 
    {
        draw_quad_line(shape_a.quad , WHITE , 5);
        for(int vertex_index = 0; vertex_index < quad_vertex_count; vertex_index++)
        {
            shape_a.quad.vertex_position[vertex_index] = Vector3Add(shape_a.quad.vertex_position[vertex_index] , velocity);
        }
        draw_quad_line(shape_a.quad , WHITE , 5);
    }
...
internal void function draw_all_collected_collision()
{
    D_game_draw();
    ...
    Vector3 frame_collision_velocity = {};
    ...
    if(!store_multiple_frame_collision)
    {
        if(frame_collision_buffer.count > 0)
        {
            ...
            if(visual_buffer_end - visual_buffer_start)
            {
                ...
                frame_collision_velocity = frame_collision.velocity;
            }
        }
    }
    ...
    if(display_all_captured_collision)
    {
        ...
        {
            draw_collision_visual(collision_visual_buffer.data[buffer_index] , frame_collision_velocity);
        }
    }
    else
    {
        if(collision_visual_buffer.count) draw_collision_visual(collision_visual_buffer.data[current_collision_visual_index] , frame_collision_velocity);
    }
    
    D_game_draw();
}

internal void function viewport_update()
{
    //TODO: these should be done once
    
    //what should be done once old me???
    if(editor_type == edit_animation)
    {
    ...
    {
        Bone * bone_array = allocate_memory_type(Bone , selected_model->bone_buffer.count , AT_frame);
        for(int bone_index = 0 ; bone_index < selected_model->bone_buffer.count ; bone_index++)
        {
    ...
    if(editor_type == edit_world || in_game)
    {
        D_game_draw();
        ...
        if(editor_type == edit_world || in_game)
        {
            for(int entity_index = 0; entity_index < client_entity_buffer.count ; entity_index++)
            {
                Entity entity = client_entity_buffer.data[entity_index];
                draw_box(entity.box , entity_color_array[entity.type]);
            }
        }
        ...
        {
            if(bounding_box_in_map_root)
            {
                BoundingBoxNode * node_stack[128] = {};
                int node_stack_count = 0;
                node_stack[node_stack_count++] = bounding_box_in_map_root;
                ...
                for(;;)
                {
                    ...
                    Box box = get_box();
                    box.position = Vector3Add(node->right_top_forward , node->left_bottom_backward);
                    box.position = Vector3Scale(box.position , 0.5f);
                    box.size = Vector3Subtract(node->right_top_forward , node->left_bottom_backward);
                    box.size = vector3_abs(box.size);
                    
                    draw_box_line(box , WHITE , 5.0f);
                }
            }
        }
        ...
        if(editor_type == edit_world)
        {
            if(current_map_edit_type == MET_box)
            {
                array_foreach(entity_index, &entity_layout_array)
                {
                    ...
                    Box entity_box = entity->box;
                    entity_box.size.x += UNIT_SIZE * 0.5f;
                    entity_box.size.y += UNIT_SIZE * 0.5f;
                    entity_box.size.z += UNIT_SIZE * 0.5f;
                    
                    draw_box(entity_box , Fade(entity_color_array[entity->type] , 0.4f));
                }
                ...
                if(last_clicked_entity)
                {
                    ...
                    draw_box_line(entity_box , Fade(WHITE , 0.4f) , 5);
                    
                    //Vector3 direction = charge_direction[last_clicked_entity->charge_direction_index];
                    //direction = Vector3Scale(direction , GRID_SIZE * 2.0);
                    //draw_arrow_ray(entity_box.position , direction , YELLOW);
                }
            }
        }
        ...
        for(int player_index = 0 ; player_index < client_player_buffer.count; player_index++)
        {
            Player player = client_player_buffer.data[player_index];
            Box player_box = get_box();
            player_box.position = player.character_position;
            player_box.size = player_box_size;
            ...
            if(player.it_is_me)
            {
                ...
                array_foreach(array_index , &camera_array)
                {
                    ...
                    {
                        if(check_shape((Vector3){} , box_to_point(player_box) , box_vertex_count , box_to_point(zone) , box_vertex_count , 0))
                        {
            ...
            Vector3 offset_to_camera = Vector3Subtract(player.position , game_camera_position);
            float offset_to_camera_length = Vector3Length(offset_to_camera);
            offset_to_camera_length -= GRID_SIZE * 2.0f;
            if(offset_to_camera_length < 0) offset_to_camera_length = 0;
            offset_to_camera = Vector3Scale(offset_to_camera , offset_to_camera_length * 0.4f);
            
            Vector3 target = Vector3Add(game_camera_position , offset_to_camera);
            
            game_camera_position = Vector3Lerp(game_camera_position , target , 0.2f);
            game_camera_offset = Vector3Lerp(game_camera_offset , last_camera_offset , 0.05f);
            
            if(player.it_is_me)
            {
                draw_box(player_box , Fade(YELLOW , 0.4f));
                draw_box_line(player.collide_box , YELLOW , 2);
                draw_box_line(player_box , YELLOW , 5);
                
                if(player.interact) draw_arrow_ray(player.position , player.interact_direction , YELLOW);
            }
            else
            {
                draw_box_line(player.collide_box , BLACK , 2);
...
internal void function game_update()
{
    ...
    editor->target_camera_zoom -= app_data->mouse_scroll_delta * 1.0f;
    ...
    base_pose_bone = allocate_memory_type( Bone , selected_model->bone_buffer.count , AT_frame);
    ...
    for(int stack_index = 0 ; stack_index < clip_bone_stack_count ; stack_index++)
    {
        ...
        current_clip_bone->bone_pose_offset = allocate_memory_type(Bone , allocate_bone_count , AT_frame);
        current_clip_bone->final_bone_pose = allocate_memory_type(Bone , allocate_bone_count , AT_frame);
        current_clip_bone->selected_bone = allocate_memory_type(bool , allocate_bone_count , AT_frame);
        current_clip_bone->hovered_bone = allocate_memory_type(bool , allocate_bone_count , AT_frame);
    ...
    for(int stack_index = 0 ; stack_index < clip_bone_stack_count ; stack_index++)
    {
        ...
        if(editor_type == edit_animation)
        {
            Bone * temp_bone = allocate_memory_type( Bone , selected_model->bone_buffer.count , AT_frame);
    ...
    if(mouse_pressing_no_check(MOUSE_BUTTON_LEFT))
    {
    ...
    for(int viewport_index = 0 ; viewport_index < viewport_count ; viewport_index++)
    {
        ...
        if(within_viewport)
        {
            ...
            {
                if(key_pressing_no_check(KEY_LEFT_SHIFT))
                {
        ...
        {
            input_state = &client_input_state;
            if(net_state.is_client) client_network_update();
        }
        
        input_state = &client_input_state;
        viewport_update();
        
        //i put this after viewport_update in order to draw stuff on top of world
        //it mean input will be delay one frame
        if(viewport_index == 0)
        {
...
internal GAME_LOOP(function game_loop)
{
    if (shader_compile_failed) return;
    
    local_persist bool decl initialized_timer = false;
    local_persist double decl update_timer = 0;
    local_persist double decl previous_loop_time = 0;
    local_persist double decl elapsed_time = 0;
    ...
    for (;elapsed_time > update_time ; elapsed_time -= update_time )
    {
        ...
        input_state = &client_input_state;
        input_state->input_array[IA_pressed_mouse].count = 0;
		input_state->input_array[IA_released_mouse].count = 0;
		input_state->input_array[IA_pressed_key].count = 0;
        input_state->input_array[IA_released_key].count = 0;
        
        for(int array_index = 0; array_index < IA_count; array_index++)
        {
            for(int key_index = 0; key_index < INPUT_MAX_KEY; key_index++)
            {
                input_state->input_array[array_index].consumed_array[key_index] = false;
            }
        }
...
internal unsigned char * function allocate_to_file_(char * name , int size , int count)
{
...
internal void function save_data_to_file(char * path)
{
...
internal void function save_game_state()
{
    ...
    WRITE_DATA(in_game , "in_game" , bool);
    WRITE_DATA(player_spawn_point , "player_spawn_point" , Vector3);
...
internal void function save_map()
{
    save_memory = malloc(MAX_SAVE_SIZE);
    data_header_array = malloc(MAX_HEADER_SIZE);
    ...
    array_foreach_B(array_index , box_index , &box_in_map_array)
    {
        ...
        WRITE_BUFFER(box.top_front , "map_box_top_front" , float , box_index , box_count);
        WRITE_BUFFER(box.top_right , "map_box_top_right" , float , box_index , box_count);
        WRITE_BUFFER(box.right_top , "map_box_right_top" , float , box_index , box_count);
        WRITE_BUFFER(box.right_front , "map_box_right_front" , float , box_index , box_count);
        WRITE_BUFFER(box.front_top , "map_box_front_top" , float , box_index , box_count);
        WRITE_BUFFER(box.front_right , "map_box_front_right" , float , box_index , box_count);
    }
    ...
    array_foreach_B(array_index , entity_index , &entity_layout_array)
    {
        ...
        WRITE_BUFFER(entity.type , "entity_type" , int , entity_index , entity_count);
        WRITE_BUFFER(entity.position , "entity_position" , Vector3 , entity_index , entity_count);
        WRITE_BUFFER(entity.box.rotation , "entity_rotation" , Quaternion , entity_index , entity_count);
        WRITE_BUFFER(entity.box.size , "entity_size" , Vector3 , entity_index , entity_count);
        WRITE_BUFFER(entity.generation_index , "entity_generation_index" , int , entity_index , entity_count);
    }
...
internal int function reassign_bone_index(int previous_bone_index)
{
...
//TODO: reassign bone index by name
internal void function load_game_state()
{
    if(!load_data_from_file(get_app_file_path(game_state_save_name))) return;
    
    READ_DATA(editor_type , "editor_type" , int);
    READ_DATA(in_game , "in_game" , bool);
    READ_DATA(player_spawn_point , "player_spawn_point" , Vector3);
    ...
    Clip * new_clip_array = allocate_memory_type(Clip , clip_count , AT_temp);
...
internal GAME_UNLOAD(function game_unload)
{
...
internal void function client_init()
{
    allocate_buffer(&all_key_frame_buffer, KeyFrame , KEY_FRAME_CAPACITY , AT_temp);
    editor = allocate_memory_type(EditorData ,1 , AT_temp);
    ...
    editor->selected_bone_stack = allocate_memory_type(BoneSelection , 256 , AT_temp);
...
internal void function game_init()
{
    ...
    entity_color_array[E_moving_wall] = YELLOW;
    entity_color_array[E_small_block] = GOLD;
    entity_color_array[E_bubble] = Fade(ORANGE , 0.4f);
    entity_color_array[E_hook] = BLUE;
    
    allocate_buffer(&bounding_box_root_stack , BoundingBoxNode * , 16 , AT_temp);
    ...
    search_queue = allocate_memory_type( Int3 , search_queue_capacity , AT_temp);
